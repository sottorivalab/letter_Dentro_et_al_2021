...
* *snps*: germinal SNPs, they can be obtained from the normal and tumour BAMS (restricted access), the input table should have the 5 following field "chr", "ref", "alt", "AD", "DP". Where "ref" is the reference allele, “alt“ the alternative and "DP" and "AD" can be obtained from the VCF. Here we provide one example file to run the vignette. 
...
*  *meta*: a table with at least a coloumn named `purity` with the actual purity values for the sample. Can be downloaded from https://dcc.icgc.org/api/v1/download?fn=/PCAWG/consensus_cnv/consensus.20170217.purity.ploidy.txt.gz
...
If you don't already have the *cna* and *meta* files, you can automatically donwload them by running this chunk
```{r, eval = FALSE}
options(timeout=100)
...
cna <-  cnas$`50e3a421-9741-4cd8-911b-0f47ca518b6a`
...
cna_p <-  cna %>% filter(!is.na(major_cn)) %>% mutate(karyotype = paste0(major_cn, ":", minor_cn), length = end - start) %>%
  mutate(is_diploid = ifelse(karyotype == "1:1", "diploid", "aneuploid"), length = end - start ) %>%
